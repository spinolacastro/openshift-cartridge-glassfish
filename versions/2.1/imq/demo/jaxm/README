...
On Windows and Linux, IMQ_HOME is the top level directory in
which the Sun Java(tm) System Message Queue product has been installed.

Note that in the Solaris Platform Edition, the different
components of Sun Java(tm) System Message Queue are distributed in the system
directories and not under a single top level 'IMQ_HOME' directory.
...
(imqbrokerd) before attempting to compile and/or run the example applications.
imqbrokerd is located in the <IMQ_HOME>/bin directory.
In the Solaris Platform Edition, imqbrokerd is located in
the /usr/bin/ directory.

Finally, note that Message Queue clients that use SOAP require JDK1.4.0 or
later.

The material in these notes is organized into the following sections:

1. List of example applications
2. Compiling the examples in a Sun Java(tm) System Message Queue environment
...
publish and subscribe capabilities of Sun Java(tm) System Message Queue.
...
Java 2 Standard Edition SDK version 1.4 (JDK1.4).

To compile the examples,
 - make sure that you are using JDK1.4, and
 - ensure that all of the following jar files are in
   your CLASSPATH -

     <IMQ_HOME>/lib/jaxm-api.jar
     <IMQ_HOME>/lib/saaj-api.jar
     <IMQ_HOME>/lib/jms.jar
     <IMQ_HOME>/lib/activation.jar
     <IMQ_HOME>/lib/imq.jar
     <IMQ_HOME>/lib/imqxm.jar

     #######jes-windev#### start ##
     On windows activation.jar and saaj-api.jar are located at following location
     <Install_Location>\share\lib\saaj-api.jar
     <Install_Location>\share\lib\activation.jar
     
     <Install_Location> is parent directory of <IMQ_HOME>
...
Note that in the Solaris Platform Edition, the jar files are
located in /usr/share/lib/
...
1.  Make the <IMQ_HOME>/demo/jaxm directory your current directory.
...
8.  To deploy the imqSOAPexamples.war file to the Sun Java(tm) System Application 
    ...
    the location of the war file for the filepath argument
        which for these instructions is <IMQ_ROOT>/demo/jaxm/imqSOAPexamples.war
...
The servlet examples  - SOAPEchoServlet and SOAPtoJMSServlet - are described below.
  ...
  - SOAPtoJMSServlet.java
    ...
    The onMessage() method of the servlet uses the Sun Java(tm) System Message 
    Queue MessageTransformer facility to convert the SOAP message to a JMS message,
...
The next two examples demonstrate how to use SOAP messages with JMS clients.
  ...
  - SendSOAPMessageWithJMS.java
    ...
    The SendSOAPMessageWithJMS example uses the Sun Java(tm) System Message Queue 
    ...
    Next, content acquired from http://java.sun.com/webservices/ is added
    to the SOAP message.
