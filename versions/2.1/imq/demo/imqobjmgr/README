...
The imqobjmgr utility allows the administrator to manage Sun Java(tm) 
System Message Queue administered objects. These include Destination
...
Its syntax is that of a java property file.  The basic format is: 
 ...
 and not of the Sun Java(tm) System Message Queue product.
...
(IMQ_HOME is the top level directory in which the Sun Java(tm) 
System Message Queue product has been installed)

On Solaris:
imqobjmgr -i /usr/demo/imq/imqobjmgr/add_topic.properties

On Linux:
imqobjmgr -i IMQ_HOME/demo/imqobjmgr/add_topic.properties
